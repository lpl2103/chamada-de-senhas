/**
 * ============================================================================
 * SISTEMA CHAMA SENHA - SCRIPT DA TV MÍDIA INDOOR (VANILLA JAVASCRIPT ES6+)
 * ============================================================================
 * Autor: Zenit Tecnologia (Modernizado por Engenheiro de Software Sênior)
 * Descrição: Script para tvads.html com reprodução de vídeos + chamadas real-time.
 * ============================================================================
 */

class ChamaSenhaTVAds {
  constructor() {
    ...
    this.dom = {
      ...
      mediaIndoorPlayer: document.getElementById('mediaIndoorPlayer')
    };
  ...
  init() {
    ...
    console.log('[ChamaSenha TV Mídia Indoor]: Inicializado.');
  }
    ...
    if (this.dom.historicoLista) {
      ...
      if (historico.length === 0) {
        ...
        historico.forEach((item) => {
          ...
          li.innerHTML = destination ? `<strong>${ticketId}</strong>${nameText} <span style="opacity:0.85;">- ${destination}</span>` : ticketId;
  ...
  anunciarVoz3Vezes(senhaStr, guicheStr, patientName = '', repeticoes = 3) {
    ...
    const falarUmaVez = () => {
      ...
      utterance.onend = () => {
        if (count < repeticoes) setTimeout(falarUmaVez, 400);
      };
  ...
  initClock() {
    const updateTime = () => {
      const now = new Date();
      if (this.dom.digitalClock) this.dom.digitalClock.textContent = now.toLocaleTimeString('pt-BR');
...
document.addEventListener('DOMContentLoaded', () => {
  window.chamaSenhaTVAds = new ChamaSenhaTVAds();
});
